...
# 4. GNOME MINIMAL (+ Tiling Assistant, KDE Connect, dbus-x11, openssl)
...
# 5. ELIMINAR BLOATWARE (CORREGIDO: sin --noautoremove inválido)
...
# 6. HERRAMIENTAS Y UTILIDADES (xclip eliminado, solo wl-clipboard para Wayland)
...
# 12. FIREWALL Y TECLADO (KDE Connect ahora sí está instalado)
...
# SEGUNDA PROTECCIÓN CRÍTICA: Forzar la instalación de Nautilus.
# Si ya está, DNF lo ignora. Si autoremove lo borró, lo restaura.
/usr/bin/dnf install -y nautilus
