#!/usr/bin/env bash
# ==============================================================================
# SCRIPT DE POST-INSTALACIÓN PARA FEDORA EVERYTHING (KDE PLASMA MINIMAL)
...
LOG_FILE="$USER_HOME/fedora_kde_install_report.log"
...
echo -e "${ANUNCIAR}=== INICIANDO CONFIGURACIÓN DE POST-INSTALACIÓN (KDE MINIMAL) ===${NC}"
echo "=== REPORTE DE POST-INSTALACIÓN DE FEDORA (KDE) ===" > "$LOG_FILE"
...
# ==============================================================================
# 4. INSTALACIÓN DE KDE PLASMA MINIMAL (Sin bloatware)
# ==============================================================================
echo -e "${ANUNCIAR}=== 4. Instalando KDE Plasma (Versión Minimalista) ===${NC}"
# Instalamos solo el núcleo de KDE y apps esenciales para ahorrar RAM en el Celeron
/usr/bin/dnf install -y \
    plasma-desktop plasma-workspace plasma-workspace-wayland plasma-workspace-x11 \
    sddm sddm-breeze \
    dolphin konsole kate ark spectacle gwenview okular kcalc \
    kdeconnect kinfocenter systemsettings \
    xdg-desktop-portal-kde packagekit-qt6 \
    qt6-qtwayland wayland-protocols-devel pipewire pipewire-pulse wireplumber
log_status $? "KDE Plasma Minimal"

# Habilitar SDDM y servicios de usuario
...
/usr/bin/dnf -y install \
    ...
    gvfs-mtp cpio git webp-pixbuf-loader unar file-roller curl cabextract \
...
/usr/bin/dnf install -y \
    ...
    fira-code-fonts rsms-inter-fonts papirus-icon-theme breeze-cursor-theme
...
# Asegurar que el sistema arranque en modo gráfico (KDE)
...
echo -e "${VERDE} Al volver, cargará SDDM para que inicies sesión en KDE Plasma.                ${NC}"
