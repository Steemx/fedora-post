...
# 4. GNOME MINIMAL (Sin bloat, SIN --now para no saltar a la GUI, + dbus-x11)
...
# Habilitar GDM para el próximo reinicio, pero NO iniciarlo ahora (sin --now)
systemctl enable gdm.service
systemctl set-default graphical.target
log_status $? "GNOME minimal instalado"

# 5. ELIMINAR BLOATWARE (Con protección para Nautilus)
...
# PROTECCIÓN CRÍTICA: Forzar reinstalación de nautilus si autoremove lo borró
...
/usr/bin/dnf -y install \
    ...
    fontconfig btop nano tailscale nautilus
