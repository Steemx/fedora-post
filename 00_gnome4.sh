...
# EJECUTAR CON: curl -sSL https://raw.githubusercontent.com/Steemx/fedora-postinstall/main/07_gnome.sh | sudo bash
# ==============================================================================
...
# 4. GNOME MINIMAL (+ dbus-x11 para gsettings, SIN --now en GDM)
...
/usr/bin/dnf install -y \
    ...
    gnome-tweaks gnome-keyring dbus-x11 \
...
# 5. ELIMINAR BLOATWARE (Con protección absoluta para Nautilus)
echo -e "${ANUNCIAR}5. Eliminando bloatware...${NC}"
/usr/bin/dnf remove --noautoremove -y \
...
# PROTECCIÓN CRÍTICA: Si autoremove fue agresivo, reinstalar Nautilus
if ! rpm -q nautilus &>/dev/null; then
    echo -e "${VERDE}⚠️ Nautilus fue eliminado por autoremove. Reinstalando...${NC}"
...
# 9. CÓDECS Y DRIVERS INTEL (Corregido: sin libvdpau-va-gl)
...
# Drivers corregidos (sin libva duplicado ni libvdpau-va-gl inexistente)
...
# 10. ZRAM Y SYSCTL (Optimización de RAM)
...
# 11. GuC/HuC INTEL (Optimización de Gráficos)
...
# 14. TWEAKS AVANZADOS DE GNOME (Rendimiento extra)
...
# 15. LIMPIEZA, DRACUT Y CONFIGURACIÓN FINAL
...
echo -e "${VERDE}curl -sSL https://raw.githubusercontent.com/Steemx/fedora-postinstall/main/02_apps.sh | sudo bash${NC}"
echo -e "${VERDE}========================================${NC}"
echo ""
