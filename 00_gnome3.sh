#!/usr/bin/env bash
# ==============================================================================
# SCRIPT 1: SISTEMA BASE GNOME MINIMAL - FEDORA 44 (CORREGIDO) Gemini
...
# 1. OPTIMIZAR DNF (Compatibilidad DNF5)
...
# 5. ELIMINAR BLOATWARE (Con protección real para Nautilus)
echo -e "${ANUNCIAR}5. Eliminando bloatware...${NC}"

# Marcar Nautilus como instalado explícitamente para que autoremove NUNCA lo borre
/usr/bin/dnf mark install nautilus
...
log_status $? "Bloatware eliminado sin afectar a Nautilus"
...
/usr/bin/dnf config-manager setopt fedora-cisco-openh264.enabled=1 || /usr/bin/dnf config-manager --set-enabled fedora-cisco-openh264
...
# Corrección de comando Tracker
sudo -u "$REAL_USER" tracker3 reset -s -r 2>/dev/null || true
...
# Ajuste seguro para CPU Nice (sin OOMScoreAdjust agresivo)
...
# Configuración mejorada de scheduler de I/O para SSDs
